...
{
    public partial class KullaniciAraziEkle : Form
    {
        string resimyolu;
        public KullaniciAraziEkle()
        {
            InitializeComponent();
        }

        private void KullaniciAraziEkle_Load(object sender, EventArgs e)
        {
        ...
        private void btnKaydet_Click(object sender, EventArgs e)
        {
            ...
            cikis = MessageBox.Show("Arazi İlanı Eklemek istediğinize emin misiniz ?", "", MessageBoxButtons.YesNo);
            if (cikis == DialogResult.Yes)
            {
                SqlBolum ekle = new SqlBolum();
                bool kontrol = false;
                kontrol = ekle.IlanAraziKayitEkle(k_Adi, cmbImar.Text, cmbIl.Text, cmbIlce.Text, txtAlan.Text, txtAda.Text, txtParsel.Text, cmbTapu.Text, txtFiyat.Text, resimyolu);
                if (kontrol == true)
                {
